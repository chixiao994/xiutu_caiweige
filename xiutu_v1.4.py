...
class AncientBookProcessor:
        ...
        try:
            ...
            result = ProcessingResult(
                ...
                debug_info={
                    ...
                    'annotated_image': self._create_annotated_preview(image, boundaries, vertical_lines, horizontal_lines, spine_points)
                }
            )
    ...
    def _create_debug_image_simple(self, image: np.ndarray, boundaries: PageBoundary,
        ...
        """创建调试图像"""
        if len(image.shape) == 3 and image.shape[2] == 3:
            debug = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)  # 转换为BGR供OpenCV使用
        else:
        ...
        return cv2.cvtColor(debug, cv2.COLOR_BGR2RGB)  # 转换回RGB
    
    def _create_annotated_preview(self, image: np.ndarray, boundaries: PageBoundary,
                                vertical_lines: List[DetectedLine], horizontal_lines: List[DetectedLine],
                                spine_points: List[Tuple[int, int]]) -> np.ndarray:
        """创建带有标记框的预览图像"""
        # 确保图像是RGB格式
        if len(image.shape) == 3 and image.shape[2] == 3:
            ...
            if image.dtype == np.uint8:
                ...
                annotated = cv2.cvtColor(image.astype(np.uint8), cv2.COLOR_BGR2RGB)
        ...
        cv2.rectangle(overlay, (boundaries.left, boundaries.top),
                     (boundaries.right, boundaries.bottom), self.colors['boundary'], 3)
        ...
        for line in vertical_lines:
            ...
            for i in range(steps):
                ...
                if i < steps:
                    cv2.line(annotated, (start_x, start_y), (end_x, end_y), color, 2)
        ...
        # 绘制书口点（用小圆圈表示）
        for point in spine_points:
            cv2.circle(annotated, point, 6, self.colors['spine'], 2)
            cv2.circle(annotated, point, 3, self.colors['spine'], -1)
        
        # 绘制中缝线
        middle_x = w // 2
        cv2.line(annotated, (middle_x, 0), (middle_x, h), self.colors['middle'], 2)
        
        # 添加文字标注
        font = cv2.FONT_HERSHEY_SIMPLEX
        cv2.putText(annotated, f"Boundary: {boundaries.width}x{boundaries.height}", 
                   (10, 30), font, 0.7, (0, 0, 0), 2)
        cv2.putText(annotated, f"Boundary: {boundaries.width}x{boundaries.height}", 
                   (10, 30), font, 0.7, (255, 255, 255), 1)
        
        cv2.putText(annotated, f"V-lines: {len(vertical_lines)}", 
                   (10, 60), font, 0.7, (0, 0, 0), 2)
        cv2.putText(annotated, f"V-lines: {len(vertical_lines)}", 
                   (10, 60), font, 0.7, (255, 255, 255), 1)
        
        cv2.putText(annotated, f"H-lines: {len(horizontal_lines)}", 
                   (10, 90), font, 0.7, (0, 0, 0), 2)
        cv2.putText(annotated, f"H-lines: {len(horizontal_lines)}", 
                   (10, 90), font, 0.7, (255, 255, 255), 1)
        ...
        # 边界框图例
        cv2.rectangle(annotated, (legend_x, legend_y), (legend_x + 150, legend_y + 120), (240, 240, 240), -1)
        cv2.rectangle(annotated, (legend_x, legend_y), (legend_x + 150, legend_y + 120), (0, 0, 0), 1)
        
        cv2.putText(annotated, "Legend:", (legend_x + 10, legend_y + 20), font, 0.5, (0, 0, 0), 1)
        
        # 边界颜色
        cv2.rectangle(annotated, (legend_x + 10, legend_y + 30), (legend_x + 30, legend_y + 45), self.colors['boundary'], -1)
        cv2.putText(annotated, "Boundary", (legend_x + 35, legend_y + 42), font, 0.4, (0, 0, 0), 1)
        
        # A面垂直线
        cv2.line(annotated, (legend_x + 10, legend_y + 50), (legend_x + 30, legend_y + 50), self.colors['a_vertical'], 2)
        cv2.putText(annotated, "A-side Vertical", (legend_x + 35, legend_y + 55), font, 0.4, (0, 0, 0), 1)
        
        # B面垂直线
        cv2.line(annotated, (legend_x + 10, legend_y + 65), (legend_x + 30, legend_y + 65), self.colors['b_vertical'], 2)
        cv2.putText(annotated, "B-side Vertical", (legend_x + 35, legend_y + 70), font, 0.4, (0, 0, 0), 1)
        
        # 书口点
        cv2.circle(annotated, (legend_x + 20, legend_y + 85), 4, self.colors['spine'], -1)
        cv2.putText(annotated, "Spine Point", (legend_x + 35, legend_y + 89), font, 0.4, (0, 0, 0), 1)
        
        # 中缝
        cv2.line(annotated, (legend_x + 10, legend_y + 100), (legend_x + 30, legend_y + 100), self.colors['middle'], 2)
        cv2.putText(annotated, "Middle Line", (legend_x + 35, legend_y + 105), font, 0.4, (0, 0, 0), 1)
...
class AncientBookGUI:
    ...
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("古籍书影智能处理系统 v4.1")
    ...
    def _create_widgets(self):
        ...
        self.notebook.add(preview_frame, text="标记预览")
    ...
    def _load_and_display_image(self, image_path, canvas):
        ...
        try:
            ...
            if canvas == self.original_canvas:
                ...
                self.annotated_image_tk = image_tk
    ...
    def _display_result(self, result, original_image_path=None):
        ...
        if result.debug_info:
            ...
            for key, value in result.debug_info.items():
                if key not in ['output_paths', 'debug_image', 'error', 'annotated_image']:
        ...
        # 显示标记预览图像
        annotated_image = result.debug_info.get('annotated_image')
        if annotated_image is not None:
            self._display_annotated_image(annotated_image)
    ...
    def _display_annotated_image(self, annotated_image_array):
        """显示带有标记框的图像"""
        try:
            # 将numpy数组转换为PIL图像
            if len(annotated_image_array.shape) == 3:
                pil_image = Image.fromarray(annotated_image_array.astype('uint8'), 'RGB')
            else:
                pil_image = Image.fromarray(annotated_image_array.astype('uint8'), 'L')
            ...
            # 切换到预览标签页
            self.notebook.select(1)  # 索引1是"标记预览"标签页
